...
func (s *Server) HandleRequest(req []byte) {
	...
	fmt.Printf("Внимание облако %s перегружено, значение предела %s%% преодолено, последнее значение метрики: %f", jsonReq.Message, jsonReq.Title, jsonReq.Alerts[0].Values.A)
}